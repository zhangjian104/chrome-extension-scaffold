...
async function runTests() {
  ...
  // T7: 图片理解（验证网关是否支持 image content block）
  console.log('\n--- T7: 图片理解（多模态） ---');
  {
    // 10x10 红色 PNG（极小体积，仅用于验证通道是否通畅）
    const RED_PIXEL_PNG =
      'iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAFklEQVQY' +
      'V2P8z8BQDwMDQwMjAxUBAAL+Af4EfqOXAAAAAElFTkSuQmCC';
    ...
    try {
      ...
      console.log(`     原始响应 content: ${JSON.stringify(res.content, null, 2).slice(0, 500)}`);
      const hasContent = Array.isArray(res.content) && res.content.length > 0;
      const textBlock = hasContent ? res.content.find((b: any) => b.type === 'text') : null;
      const hasText = !!textBlock && typeof textBlock.text === 'string' && textBlock.text.length > 0;

      if (hasText) {
        assert(true, '网关支持图片：返回了有效文本描述');
        console.log(`     回复: ${textBlock!.text.slice(0, 120)}`);
        console.log('     ✨ 结论：网关支持图片传入（多模态可用）');
      } else {
        assert(true, '网关接受了请求但未返回有效文本（图片被忽略）');
        console.log('     ⚠️ 结论：网关不支持图片理解，返回了空文本块，仅支持纯文本');
      }
    } catch (e: any) {
      assert(true, '网关拒绝了图片请求（已记录，非阻断性错误）');
      console.log(`     ⚠️ 请求被拒绝: ${e.message?.slice(0, 150)}`);
      console.log('     结论：网关不支持图片传入');
    }
  }
